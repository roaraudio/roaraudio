...
install: prep-install-dirs
	...
	sh -c 'cd lib; for file in $(COMP_LIB); do for i in "" .{0,1}; do ln -fs '$(DESTDIR)$(PREFIX_LIB)'/$$file$(SHARED_SUFFIX) '$(DESTDIR)$(PREFIX_COMP)'/`echo $$file | sed s/roar//`$(SHARED_SUFFIX)$$i; done; done'
...
semi-install: prep-install-dirs
	...
	sh -c 'cd lib; for file in $(COMP_LIB); do for i in "" .{0,1}; do ln -fs `pwd`/$$file$(SHARED_SUFFIX) '$(DESTDIR)$(PREFIX_COMP)'/`echo $$file | sed s/roar//`$(SHARED_SUFFIX)$$i; done; done'
