...
void roar_notify_proxy_std(struct roar_notify_core * core, struct roar_event * event, void * userdata) {
 struct {
  ...
  {.event_min = ROAR_NOTIFY_EGRP2EVENT(ROAR_NOTIFY_OE_GROUP_CLIENTS),
   .event_max = ROAR_NOTIFY_EGRP2EVENT(ROAR_NOTIFY_OE_GROUP_CLIENTS + 0xff),
   .events = {
  ...
  {.event_min = ROAR_NOTIFY_EGRP2EVENT(ROAR_NOTIFY_OE_GROUP_SOURCES),
   .event_max = ROAR_NOTIFY_EGRP2EVENT(ROAR_NOTIFY_OE_GROUP_SOURCES + 0xff),
   .events = {
    ROAR_EGRP_ANY_SOURCE_EVENT,
    EOL
   }
  },
  {.event_min = ROAR_NOTIFY_EGRP2EVENT(ROAR_NOTIFY_OE_GROUP_SAMPLES),
   .event_max = ROAR_NOTIFY_EGRP2EVENT(ROAR_NOTIFY_OE_GROUP_SAMPLES + 0xff),
   .events = {
